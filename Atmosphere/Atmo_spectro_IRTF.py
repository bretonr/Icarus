...
class Atmo_IRTF_spectro(Atmo_grid):
    ...
    def Flux_init(self, flns, oversample=None, sigma=None, tophat=None, thin=None, wave_cut=None, temp_cut=None, logg_cut=None, convert=None, linlog=False, verbose=False):
        ...
        for i,l in enumerate(lst[:,0]):
            ...
            tmp = Read_IRTF(flns[int(l)], oversample=oversample, sigma=sigma, tophat=tophat, thin=thin, wave_cut=wave_cut, convert=convert, linlog=linlog)
            grid.append(tmp[0])
            wav.append(tmp[1])
            self.z0 = tmp[2]
...
######################## Utilities for Atmo_IRTF_spectro ########################
def Read_IRTF(fln, oversample=None, sigma=None, tophat=None, thin=None, wave_cut=None, convert=None, linlog=False):
    """Read_IRTF(fln, oversample=None, sigma=None, tophat=None, thin=None, wave_cut=None, convert=None, linlog=False)
    ...
    >>> grid, wav, z = Read_IRTF(fln, thin=20)
    """
    ...
    #grid = (2 / cts.pi / numpy.sqrt(3)) * 10**grid
    #grid = (2 / cts.pi / numpy.sqrt(3)) * grid
